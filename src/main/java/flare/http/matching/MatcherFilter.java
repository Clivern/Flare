...
import flare.HaltException;
import flare.RequestResponseFactory;
import flare.Response;
import flare.embeddedserver.jetty.HttpRequestWrapper;
import flare.route.HttpMethod;
import flare.serialization.SerializerChain;
import flare.staticfiles.StaticFilesConfiguration;

/**
 * Matches Flare routes and filters.
 *
 * @author Per Wendel
 */
public class MatcherFilter implements Filter {
    ...
    private flare.route.Routes routeMatcher;
     ...
     * @param externalContainer Tells the filter that Flare is run in an external web container.
     *                          If true, chain.doFilter will be invoked if request is not consumed by Flare.
     * @param hasOtherHandlers  If true, do nothing if request is not consumed by Flare in order to let others handlers process the request.
     */
    public MatcherFilter(flare.route.Routes routeMatcher,
    ...
    public void doFilter(ServletRequest servletRequest,
        ...
        try {
            ...
            if (body.notSet()) {
                LOG.info("The requested route [{}] has not been mapped in Flare for {}: [{}]",
