...
import flare.embeddedserver.EmbeddedServer;
import flare.embeddedserver.EmbeddedServers;
import flare.embeddedserver.jetty.websocket.WebSocketHandlerClassWrapper;
import flare.embeddedserver.jetty.websocket.WebSocketHandlerInstanceWrapper;
import flare.embeddedserver.jetty.websocket.WebSocketHandlerWrapper;
import flare.route.HttpMethod;
import flare.route.Routes;
import flare.route.ServletRoutes;
import flare.routematch.RouteMatch;
import flare.ssl.SslStores;
import flare.staticfiles.MimeType;
import flare.staticfiles.StaticFilesConfiguration;

import static java.util.Objects.requireNonNull;
import static flare.globalstate.ServletFlag.isRunningFromServlet;

/**
 * Represents a Flare server "session".
 * If a user wants multiple 'Flares' in his application the method {@link Service#ignite()} should be statically
...
public final class Service extends Routable {
    private static final Logger LOG = LoggerFactory.getLogger("flare.Flare");
    ...
    /**
     * Creates a new Service (a Flare instance). This should be used instead of the static API if the user wants
    ...
    /**
     * Set the IP address that Flare should listen on. If not called the default
    ...
    /**
     * Set the port that Flare should listen on. If not called the default port
    ...
    /**
     * Retrieves the port that Flare is listening on.
     *
     * @return The port Flare server is listening on.
    ...
    /**
     * Waits for the flare server to be initialized.
    ...
    /**
     * Stops the Flare server and clears all routes.
     */
    ...
    /**
     * Waits for the Flare server to stop.
    ...
    /**
     * Sets Flare to trust the HTTP headers that are commonly used in reverse proxies.
    ...
    /**
     * Sets Flare to NOT trust the HTTP headers that are commonly used in reverse proxies.
     * More info at https://www.eclipse.org/jetty/javadoc/current/org/eclipse/jetty/server/ForwardedRequestCustomizer.html
     */
