...
import flare.Flare;
import flare.examples.exception.NotFoundException;
import flare.util.FlareTestUtil;

import static flare.Flare.exception;
import static flare.Flare.get;
import static flare.Flare.staticFiles;
...
public class StaticFilesTest {
    ...
    public void testMimeTypes() throws Exception {
        ...
        Assert.assertEquals("image/png", doGet("/img/flarelogo.png").headers.get("Content-Type"));
        Assert.assertEquals("image/svg+xml", doGet("/img/flarelogo.svg").headers.get("Content-Type"));
        Assert.assertEquals("application/octet-stream", doGet("/img/flarelogoPng").headers.get("Content-Type"));
        Assert.assertEquals("application/octet-stream", doGet("/img/flarelogoSvg").headers.get("Content-Type"));
    ...
    public void testStaticFileCssStyleCss() throws Exception {
        FlareTestUtil.UrlResponse response = doGet("/css/style.css");
    ...
    public void testStaticFilePagesIndexHtml() throws Exception {
        FlareTestUtil.UrlResponse response = doGet("/pages/index.html");
        Assert.assertEquals(200, response.status);
    ...
    public void testStaticFilePageHtml() throws Exception {
        FlareTestUtil.UrlResponse response = doGet("/page.html");
        Assert.assertEquals(200, response.status);
    ...
    public void testDirectoryTraversalProtectionLocal() throws Exception {
        String path = "/" + URLEncoder.encode("..\\flare\\", "UTF-8") + "Flare.class";
        FlareTestUtil.UrlResponse response = doGet(path);
    ...
    public void testExternalStaticFile() throws Exception {
        FlareTestUtil.UrlResponse response = doGet("/externalFile.html");
        Assert.assertEquals(200, response.status);
    ...
    private static void testGet() throws Exception {
        FlareTestUtil.UrlResponse response = testUtil.doMethod("GET", "/hello", "");
    ...
    public void testExceptionMapping404() throws Exception {
        FlareTestUtil.UrlResponse response = doGet("/filethatdoesntexist.html");

        Assert.assertEquals(404, response.status);
        Assert.assertEquals(NOT_FOUND_BRO, response.body);
    }

    private FlareTestUtil.UrlResponse doGet(String fileName) throws Exception {
